...
# --- CONFIGURATION ---
ZIP_FILE_NAME = "AniDex.zip"  # The name of the zip file you bundle
APP_EXE_NAME = "AniDex.exe" # The name of the executable inside the zip
        ...
        if self.current_step == 0:
            ...
            if self.create_shortcut_var.get():
                ...
                if target_app:
                    ...
                    shortcut_path = os.path.join(desktop_dir, "AniDex.lnk")  #here
                    try:
            ...
            try:
                ...
                os.environ["APPDATA"],
                r"Microsoft\Windows\Start Menu\Programs\AniDex"
                )
                os.makedirs(start_menu_dir, exist_ok=True)

                start_menu_shortcut = os.path.join(start_menu_dir, "AniDex.lnk")

                create_shortcut(target_app, start_menu_shortcut)

            except Exception as e:
                messagebox.showwarning("Start Menu Error", f"Failed to create Start Menu shortcut:\n{str(e)}"
)
            ...
            if self.create_shortcut_var.get():
                ...
                desktop_dir = os.path.join(os.path.expanduser("~"), "Desktop")
                shortcut_path = os.path.join(desktop_dir, "AniDex.lnk")
                try:
                    create_shortcut(target_app, shortcut_path)
                except Exception as e:
                    messagebox.showwarning("Shortcut Error", f"Failed to create shortcut:\n{str(e)}")
